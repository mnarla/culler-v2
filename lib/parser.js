...
// ── Schema fingerprint ───────────────────────────────────────────────────────
// The expected path into the Pathfinder response. If any of these keys are
// missing from a new Spotify client version, SCHEMA_MISMATCH fires.
...
export function mergePlaylistBatches(existing, batch) {
  return {
    ...
    playlistId: existing.playlistId || batch.playlistId,
    playlistName: existing.playlistName || batch.playlistName,
    totalExpected: batch.totalExpected ?? existing.totalExpected,
    tracks: [...existing.tracks, ...batch.tracks],
    skippedRows: existing.skippedRows + batch.skippedRows,
    schemaMismatch: existing.schemaMismatch || batch.schemaMismatch,
