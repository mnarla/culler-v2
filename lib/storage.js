...
/**
 * Reviewed Tracks History (Tracks already culled or explicitly kept)
 */
export async function getReviewedTracks() {
  return getStorageItem(STORAGE_KEYS.REVIEWED_TRACKS, {
    culled: [], // Array of track identifiers
    kept: [],   // Array of { name, artist, reason }
  });
}

export async function setReviewedTracks(reviewed) {
  return setStorageItem(STORAGE_KEYS.REVIEWED_TRACKS, reviewed);
}
