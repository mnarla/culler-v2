...
export function buildBatchScoringPrompt(tracks, activeRules, playlistName) {
  ...
  lines.push(
    ...
    'To optimize user review and capture candidates across large playlists, identify SKIP candidates ' +
    'and classify each candidate into one of two confidence tiers: ' +
    ...
    'Return at most the top 50 strongest SKIP candidates (confidence ≥ 60%), sorted by confidence descending. ' +
    'Ensure variety across different artists and eras rather than dominating the report with a single artist. ' +
    'Do NOT output entries for tracks you decide to KEEP (confidence < 60%). If no tracks should be skipped, return an empty array [].'
  );
  lines.push('');
