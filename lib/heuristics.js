 ...
 *
 *   buildBatchScoringPrompt(unplayedTracks, activeRules, playlistName)
 *     → Prompt string for batch skip/keep prediction on unplayed tracks.
 *
 *   buildBatchScoringPromptForChunk(chunkTracks, activeRules, playlistName, playlistProfile)
 *     → Prompt string for chunked batch skip prediction with playlist profile context.
 *
 * JSON Output Contracts:
 *   Calibration → { updatedRules: [...], changes: [...] }
 *   Batch Scoring → [ { originalIndex, name, artist, decision, confidence, tier, reason } ]
 */
...
export function buildBatchScoringPrompt(tracks, activeRules, playlistName) {
  return buildBatchScoringPromptForChunk(tracks, activeRules, playlistName);
}
...
export function buildBatchScoringPromptForChunk(chunkTracks, activeRules, playlistName, playlistProfile) {
  ...
  const dynamicCap = Math.max(20, Math.ceil(chunkTracks.length * 0.35));
