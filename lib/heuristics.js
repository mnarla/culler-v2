...
export function buildBatchScoringPrompt(tracks, activeRules, playlistName) {
  ...
  lines.push(
    'Evaluate all tracks above against the listener taste guidelines. ' +
    'To optimize response size and user review, return at most the top 40 strongest SKIP candidates (confidence ≥ 70%), sorted by confidence descending. ' +
    'Do NOT output entries for tracks you decide to KEEP. If no tracks should be skipped, return an empty array [].'
  );
  lines.push('');
