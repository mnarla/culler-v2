...
/**
 * Build a rule synthesis prompt from a cluster of mispredictions.
 *
 * Ported from Culler v1's _build_synthesis_prompt in consolidate_rules.py.
 * Used when the calibration loop identifies a pattern of repeated errors
 * and needs to generate a new corrective rule.
 *
 * @param {Object} cluster
 * @param {string} cluster.patternDescription - Human-readable description of the shared pattern.
 * @param {Object[]} cluster.misses - Array of miss objects with predicted/actual/reasoning fields.
 * @returns {string} Complete rule synthesis prompt.
 */
export function buildRuleSynthesisPrompt(cluster) {
  const reasoningBlock = cluster.misses
    .slice(0, 10) // Cap at 10 examples to stay within context
    .map((m, i) =>
      `  [${i + 1}] (predicted ${m.predicted}, actual ${m.actual}) ` +
  ...
  return [
    ...
    'The skip-prediction model consistently makes errors on a cluster of tracks sharing this pattern:',
    '',
    ...
    '  - Reference the specific attributes that matter, not just the pattern label',
...
/**
 * Build a merge prompt for two near-duplicate rules.
 *
 * Ported from Culler v1's _build_merge_prompt in consolidate_rules.py.
 *
 * @param {string} ruleA - Text of the first rule.
 * @param {string} ruleB - Text of the second rule.
 * @returns {string} Complete rule merge prompt.
 */
export function buildRuleMergePrompt(ruleA, ruleB) {
  return [
